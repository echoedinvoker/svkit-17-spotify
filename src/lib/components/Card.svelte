<script lang="ts">
	...
	type Album = SpotifyApi.AlbumObjectSimplified | SpotifyApi.AlbumObjectFull;
	type Playlist = SpotifyApi.PlaylistObjectSimplified | SpotifyApi.PlaylistObjectFull;
	type Artist = SpotifyApi.ArtistObjectFull;

	export let item: Album | Playlist | Artist;
</script>
...
<div class="card {item.type}">
	...
	{#if item.type === 'album'}
		<p class="truncate-1">by {item.artists.map((artist) => artist.name).join(', ')}</p>
	{/if}
